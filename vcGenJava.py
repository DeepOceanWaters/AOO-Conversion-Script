...
def parseNext(str,moduleName):
        ...
        #Obj Tuple = Name, inFiles, outFiles, args
                if (type != "null"):
                        ...
                        args = args.strip()
                        args = fixPaths(args)
                        tmp_it = args.find("-Dtarget=")+len("-Dtarget=")
                        end_it = args.find(" ",tmp_it)
                        targetName = args[tmp_it:end_it]
                #Don't want to do anything with this
                        if (prjName == "inc"):
                                continue
                        patchVCProjJava(prjName,moduleName,targetName)
                else:
                        break
        return it
...
def patchVCProjJava(prjName,moduleName,targetName):
        ...
        origFile = origFile.replace(targetStr,targetName)
...
def patchVCProjMake(prjName,isPostBuild,buildCmd,rebuildCmd,cleanCmd):
        ...
        prjTuple = prjName.strip()+"_"+buildType, prjId, dependencies
